...
function initDb() {
  ...
  if (!hasTesti) {
    const stmt = db.prepare(
      'INSERT INTO testimonials (id, name, company, photo, message, rating) VALUES (?, ?, ?, ?, ?, ?)'
    )
    stmt.run(['1', 'Budi Santoso', 'PT. Maju Jaya', 'https://images.unsplash.com/photo-1556740714-a8395b3bf30f', 'Layanan yang sangat profesional dan cepat. Pengiriman spare parts kami selalu tepat waktu dan dalam kondisi sempurna.', 5])
    stmt.run(['2', 'Siti Nurhaliza', 'CV. Sentosa', 'https://images.unsplash.com/photo-1556740714-a8395b3bf30f', 'Sudah 3 tahun kami menggunakan jasa mereka untuk e-commerce fulfillment. Sistem tracking yang real-time sangat membantu bisnis kami.', 5])
    stmt.free()
  }
  ...
  const defaultHash = sha256('admin123')
...
app.post('/api/shipments', (req, res) => {
  ...
  try {
    ...
    stmt.run([id, trackingNumber, customer, origin, destination, weight, status, courier, createdDate, estimatedDelivery, notes])
    stmt.free()
    saveDb()
...
app.put('/api/shipments/:id', (req, res) => {
  const { id } = req.params
  const { trackingNumber, customer, origin, destination, weight, status, courier, createdDate, estimatedDelivery, notes } = req.body || {}
  
  try {
    const stmt = db.prepare(
      'UPDATE shipments SET trackingNumber=?, customer=?, origin=?, destination=?, weight=?, status=?, courier=?, createdDate=?, estimatedDelivery=?, notes=? WHERE id=?'
    )
    stmt.run([trackingNumber, customer, origin, destination, weight, status, courier, createdDate, estimatedDelivery, notes, id])
    stmt.free()
    saveDb()
    res.json({ ok: true })
  } catch (e) {
...
app.get('/api/services', (req, res) => {
  const rows = execAll('SELECT * FROM services ORDER BY id ASC')
  res.json(rows)
})
...
app.post('/api/users', (req, res) => {
  ...
  try {
    ...
    const pwdHash = password ? sha256(password) : sha256('admin123')
    const stmt = db.prepare(
      'INSERT INTO users (id, name, email, role, permissions, lastActive, status, passwordHash) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    )
